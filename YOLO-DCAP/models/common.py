...
class MDRB(nn.Module):
    """
    Multi Dilated Residual Block (MDRB)
    Incorporates multiple parallel dilated convolutions with different dilation rates
    to capture features at different scales simultaneously.
    """
    def __init__(self, c1, c2, k=3, s=1, dilation_rates=[2, 3]):
        super(MDRB, self).__init__()
        self.c1 = c1
        self.c2 = c2
        
        # 1x1 conv to adjust channels if needed
        self.conv_match = nn.Conv2d(c1, c2, 1, 1, 0, bias=False) if c1 != c2 else nn.Identity()
        
        # Multiple parallel dilated convolutions
        self.dilated_convs = nn.ModuleList()
        
        # Calculate the output channels for each dilated convolution
        split_channels = c2 // len(dilation_rates)
        remainder = c2 % len(dilation_rates)
        channels = [split_channels + (1 if i < remainder else 0) for i in range(len(dilation_rates))]
        ...
        for i, d in enumerate(dilation_rates):
            ...
            self.dilated_convs.append(
                nn.Sequential(
                    nn.Conv2d(c1, channels[i], k, s, padding=padding, dilation=d, bias=False),
...
class SSCA(nn.Module):
    """
    Simplified Spatial and Channel Attention (SSCA)
    Computes spatial and channel attention simultaneously in a single, efficient operation.
    """
    def __init__(self, c):
    ...
    def forward(self, x):
        ...
        # Channel attention
        avg_pool = self.avg_pool(x).view(batch_size, c)
        channel_attention = self.channel_mlp(avg_pool).view(batch_size, c, 1, 1)
...
class ConvMDRB(nn.Module):
    """
    Replacement for Conv module in YOLOv5, using MDRB followed by SSCA
    """
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super(ConvMDRB, self).__init__()
        self.mdrb = MDRB(c1, c2, k, s)
        self.ssca = SSCA(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
    
    def forward(self, x):
        return self.act(self.ssca(self.mdrb(x)))


class BottleneckAtt(nn.Module):
    """
    Standard bottleneck with SSCA attention mechanism
    """
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
        super(BottleneckAtt, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = nn.Conv2d(c1, c_, 1, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(c_)
        self.cv2 = nn.Conv2d(c_, c2, 3, 1, padding=1, bias=False)
...
class C3Att(nn.Module):
    """
    C3 module with SSCA attention mechanism
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super(C3Att, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = nn.Conv2d(c1, c_, 1, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(c_)
        ...
        # Create bottleneck modules with attention
        self.m = nn.Sequential(*[BottleneckAtt(c_, c_, shortcut, g, e=1.0) for _ in range(n)])
        
        # Apply SSCA at the end
        self.ssca = SSCA(c2)
    ...
    def forward(self, x):
        ...
        out = self.act(self.bn3(self.cv3(out)))
        out = self.ssca(out)
        
        return out

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Custom blocks added here %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
import torch
import torch.nn as nn
import torch.nn.functional as F
import math
...
class CBAM(nn.Module):
    ...
    As described in "CBAM: Convolutional Block Attention Module" (Woo et al., 2018)
...
class ChannelAttention(nn.Module):
    ...
    def __init__(self, c1, reduction_ratio=16):
        ...
        self.mlp = nn.Sequential(
            nn.Conv2d(c1, c1 // reduction_ratio, kernel_size=1, bias=False),
            nn.ReLU(inplace=True),
            nn.Conv2d(c1 // reduction_ratio, c1, kernel_size=1, bias=False)
        )
    ...
    def forward(self, x):
        ...
        # Combine features and apply sigmoid
...
class SpatialAttention(nn.Module):
    ...
    def __init__(self, kernel_size=7):
        ...
        # Ensure padding is appropriate for maintaining spatial dimensions
    ...
    def forward(self, x):
        # Generate average and max feature maps along channel dimension
        avg_out = torch.mean(x, dim=1, keepdim=True)
        max_out, _ = torch.max(x, dim=1, keepdim=True)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        # Project and reshape: (B, C, H, W) -> (B, num_patches, embed_dim)
        x = self.proj(x)
        B, C, H, W = x.shape
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, mlp_ratio=4., drop=0., attn_drop=0.):
        ...
        self.mlp = MLP(in_features=embed_dim, hidden_features=mlp_hidden_dim, out_features=embed_dim, drop=drop)
...
class ViT(nn.Module):
    ...
    Simplified implementation for integration with YOLOv5
    """
    def __init__(self, img_size=224, patch_size=16, in_c=3, embed_dim=768, 
    ...
    def forward_features(self, x):
        ...
        # Remove class token for feature map reconstruction
        x = x[:, 1:]
        
        # Reshape back to feature map (B, H, W, C)
...
# Integration modules for YOLOv5

class C3CBAM(nn.Module):
    """
    C3 module with CBAM integration
    """
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
...
class ViTBlock(nn.Module):
    """
    ViT Block for integration with YOLOv5
    Processes a feature map with Vision Transformer and returns feature map of same dimensions
    """
    def __init__(self, c1, embed_dim=256, depth=4, num_heads=8, patch_size=4):
        super(ViTBlock, self).__init__()
        
        # Adjust patch size based on typical input size from YOLOv5 backbone
        self.patch_size = patch_size
        
        # Input projection to match embed_dim if necessary
        self.proj_in = nn.Conv2d(c1, embed_dim, 1, 1) if c1 != embed_dim else nn.Identity()
        
        # Vision Transformer
        self.vit = ViT(
            img_size=None,  # Will be determined at runtime
        ...
        # Output projection back to original channels if necessary
        self.proj_out = nn.Conv2d(embed_dim, c1, 1, 1) if c1 != embed_dim else nn.Identity()
        
        # Residual connection flag
        self.use_residual = (c1 == embed_dim)
    ...
    def forward(self, x):
        ...
        # Note: The ViT implementation needs to handle dynamic image sizes
        x = self.vit(x)
...
# Bottleneck from YOLOv5 for use in C3CBAM
class Bottleneck(nn.Module):
    """
    Standard bottleneck from YOLOv5
    """
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):
...
# Utility function to ensure trunc_normal_ init is available
if not hasattr(nn.init, 'trunc_normal_'):
    def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
        ...
        # Uniformly fill tensor with values from [l, u], then transform to
        # [2l-1, 2u-1].
